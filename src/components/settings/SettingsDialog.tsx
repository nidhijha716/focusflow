  ...
  return (
    <Dialog open={open} onClose={onClose} labelledBy={titleId}>
      ...
      <div className="mt-4 flex flex-col gap-6">
        ...
        <fieldset className="flex flex-col gap-3">
          <legend className="text-sm font-semibold text-text-primary">Session-end alarm</legend>
          <p className="text-xs text-text-secondary">
            Pick a Spotify track to play once when a focus or break session completes.
          </p>
          <div className="rounded-xl border border-border bg-surface-soft p-3">
            <SpotifyTrackSearch
              ...
              onSelect={(track) => {
                settings.setAlarmSource("spotify");
                settings.setSpotifyAlarmTrack(toSpotifyTrackRef(track));
              }}
              onClear={() => settings.setSpotifyAlarmTrack(null)}
            />
            {settings.spotifyAlarmTrack ? (
              <p className="mt-2 text-xs text-[#1db954]">
                Plays at session end: {settings.spotifyAlarmTrack.name} — {settings.spotifyAlarmTrack.artist}
              </p>
            ) : (
              <p className="mt-2 text-xs text-text-secondary">No alarm track selected. The built-in beep is used as fallback.</p>
            )}
          </div>
        </fieldset>
